...
func TestRouterPublicAPIs(t *testing.T) {
	Convey("Given an api router and proxies with all public endpoints available", t, func() {
		...
		Convey("A request to a feedback subpath is proxied to feedbackAPI", func() {
			w := createRouterTest(cfg, "http://localhost:23200/v1/feedback")
			So(w.Code, ShouldEqual, http.StatusOK)
			verifyProxied("/feedback", feedbackAPIURL)
		})
