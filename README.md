...
| DEPRECATION_CONFIG_FILE                  | _unset_                  | Optional path to a seperate deprecations config file loaded at startup (see below for detais) |
...
of deprecated paths. If this environment variable is unset, then the deprecation middleware is disabled. Otherwise,
these paths will then be handled by middleware that adds deprecation headers as well as returning a custom error message
and NOT_FOUND(404) status during defined outage periods.
